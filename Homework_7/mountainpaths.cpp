...
int main() {
    ...
    //coloring all paths and finding shortest path 
    int min_change = colorPath(griddata, r_val, g_val, b_val, 252, 25, 63, 0);
    int temp;
    int index = 0;
    for(i = 0; i < numrows; ++i){
        temp = colorPath(griddata, r_val, g_val, b_val, 252, 25, 63, i);
        if(temp < min_change){
            min_change = temp;
            index = i;
        }
    ...
    //making shortest path green
    int min_elevation_change = colorPath(griddata, r_val, g_val, b_val, 31, 253, 13, index);
